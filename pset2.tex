...
% \textcolor{red}{This problem is incorrect. Gaussian rounding does not work, because $\sum z_i$ is not $0$ in expectation. The correct algorithm does something fancier by thresholding the Gaussian more carefully (instead of just based on sign) to return a vector that is balanced in expectation. The difficulty is then showing that the variance is small, which requires a more involved argument than the usual GW one.}
\begin{problem}
